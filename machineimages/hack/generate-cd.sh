...
component-cli ca create "${CA_PATH}" \
    --component-name=github.com/gardener/landscaper-utils/"${COMPONENT_NAME}" \
    --component-version=${EFFECTIVE_VERSION} \
    --repo-ctx=${REPO_CTX}

echo "> Extending resources.yaml: adding image of ${COMPONENT_NAME}"
...
access:
  type: ociRegistry
  imageReference: eu.gcr.io/gardener-project/landscaper-utils/${COMPONENT_NAME}:${EFFECTIVE_VERSION}
...
EOF

echo "> Creating ctf"
CTF_PATH="$(mktemp -d)"/ctf.tar
CTF_PATH=${CTF_PATH} \
    BASE_DEFINITION_PATH=${BASE_DEFINITION_PATH} \
    CURRENT_COMPONENT_REPOSITORY=${REPO_CTX} \
    RESOURCES_FILE_PATH=${RESOURCES_FILE_PATH} \
    bash $SOURCE_PATH/../.ci/component_descriptor
